(function (window) {
	...
	function safeEval(code) {
		...
		if (code === "") {
			...
			if (code) {
				...
				for (var i in window) {
					windowClone[i] = window[i];
				}

				eval(safeCode);
				for (var i in data._globalStore) {
					data[i] = data._globalStore[i];
				}

				for (var i in window) {
					if (windowClone[i] === undefined &&
						i !== "__expression__") {
						data[i] = window[i];
						delete window[i];
					}
	...
	function extractVariables (code) {
		// Good enough for hackathon. Probably gets more fn names than it should
		var tokens = code.split(/\s|\(|\)/g);
		var potentialVariables = [];

		for (var i = 0; i < tokens.length; i++) {
			if (tokens[i].indexOf("function") !== -1) {
				potentialVariables.push(tokens[i + 1]);
			} else if (tokens[i].indexOf("var") !== -1) {
				potentialVariables.push(tokens[i + 1]);
			}
		}
